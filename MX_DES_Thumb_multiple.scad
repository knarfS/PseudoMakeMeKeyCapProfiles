...
num_caps = 3;      // <-- number of caps to render in a row
cap_spacing = 19.05;  // center-to-center spacing between caps (adjust to taste)

// Render-time parameters passed into keycap() when multi-rendering
base_KeyID = 13;    // which KeyID profile to use for all caps, refer to KeyParameters Struct
render_cutLen = 0; //Don't change. for chopped caps
render_Stem = true; //turn on shell and stems
render_Dish = true; //turn on dish cut
render_Stab = 0;
render_visualizeDish = false; // turn on debug visual of Dish 
render_crossSection = false; // center cut to check internal
render_homeDot = false; //turn on homedots
render_Legends = false;

// config values for conntecting rods
side_y_offset = 0;   // positive moves toward +Y, negative toward -Y
connector_stem_dia = 2;    // mm (peg diameter)
connector_stem_len = 5;    // mm (peg height)
connector_link_dia = 3;    // mm (link cylinder diameter)
connector_fn = 32;         // resolution for stems and link cylinders
// how far outward from the cap center to place the side peg (additional margin)
side_margin = 2; // mm extra gap outside the cap half-extent


/* Render multiple keycaps for printing */
//multi_keycaps(num_caps, base_KeyID, true);
...
// Note: ChamfFinRad and ChamfInitRad referenced earlier in original script weren't defined
// If you rely on CapRadius, define ChamfFinRad and ChamfInitRad or modify accordingly.
...
// ----------------- NEW: side-mounted connector functions -----------------

// compute per-cap side offset (distance from cap center to outer wall) and add a margin
function side_offset_for(keyID) = max(BottomWidth(keyID), BottomLength(keyID))/2 + side_margin;

// create a vertical side peg attached to the cap's outer wall.
// xpos = x center of the cap (global coordinate), keyID for that cap
module connector_stem_at(xpos = 0, keyID = base_KeyID, y_sign = 1) {
    y = y_sign * side_offset_for(keyID); // allow placing on +Y or -Y side
    // Put the peg base at z = -connector_stem_len so top is at z=0 (flush with cap bottom)
    translate([xpos, y, -connector_stem_len])
        cylinder(h = connector_stem_len, d = connector_stem_dia, $fn = connector_fn);
}

// horizontal linking cylinder between the side pegs of cap at x1 and cap at x2.
// both pegs are expected at the same Y side (y_sign). We put the cylinder centered at half-height of pegs.
module connector_link_between(x1 = 0, x2 = cap_spacing, keyID1 = base_KeyID, keyID2 = base_KeyID, y_sign = 1) {
    y = y_sign * side_offset_for(keyID1); // assume same side for both caps
    dx = x2 - x1;
    // place the link at z = -connector_stem_len/2 (mid-height of the pegs) and run along +X
    translate([x1, y, -connector_stem_len/2])
        rotate([0,90,0]) // rotate so cylinder axis aligns along +X
            cylinder(h = dx, d = connector_link_dia, $fn = connector_fn);
}

// ----------------- NEW: multi-cap assembly (side-mounted connectors) -----------------
module multi_keycaps(num = 1, base_KeyID = base_KeyID, all_on_positive_side = true) {
    // Render the caps in a row along +X
    for (i=[0:num-1]) {
        translate([i*cap_spacing, 0, 0])
            mirror([0,0,0])
                keycap(keyID = base_KeyID, cutLen = render_cutLen, visualizeDish = render_visualizeDish,
                   rossSection = false, Dish = render_Dish, Stem = render_Stem,
                   crossSection = render_crossSection, Legends = render_Legends,
                   homeDot = render_homeDot, Stab = render_Stab);
    }

    // If only one cap, no connector parts are added
    if (num > 1) {
        // place side pegs; choose which side with all_on_positive_side flag
        y_sign = all_on_positive_side ? 1 : -1;

        for (i=[0:num-1]) {
            connector_stem_at(i*cap_spacing, base_KeyID, y_sign);
        }

        // place horizontal link segments between adjacent pegs
        for (i=[0:num-2]) {
            connector_link_between(i*cap_spacing, (i+1)*cap_spacing, base_KeyID, base_KeyID, y_sign);
        }
    }
}